...
void APMFreshFlashUITest::_verifyFramePrereq(const QString& compObjectName, bool expectPrereqShown)
{
    ...
    QQuickItem* messagePanel =
        findVisibleItem(_rootItem, QStringLiteral("vehicleConfig_messagePanel"), expectPrereqShown ? 3000 : 0);
...
void APMFreshFlashUITest::_verifySetupIndicator(const QString& compObjectName, bool expectSetupComplete)
{
    ...
    QVERIFY2(QTest::qWaitFor(
                 [&] { return compBtn && (compBtn->property("setupComplete").toBool() == expectSetupComplete); }, 5000),
             qPrintable(QStringLiteral("%1 setupComplete property never became %2")
                            .arg(compObjectName)
                            .arg(expectSetupComplete ? "true" : "false")));
    ...
    QVERIFY2(iconColor.isValid(), qPrintable(QStringLiteral("%1 icon.color could not be read").arg(compObjectName)));
...
void APMFreshFlashUITest::_testFreshFlashSetupState()
{
    ...
    expectAppMessage(
        QRegularExpression(QStringLiteral("Configuration tasks remain before this vehicle is ready to fly")));
    runWithMockLink(
        ...
        [&](QPointer<MockLink> /*mockLink*/, Vehicle* vehicle) {
            ...
            QCOMPARE(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("FRAME_CLASS"))
                         ->rawValue()
                         .toInt(),
                     0);
            QVERIFY2(
                qFuzzyIsNull(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("COMPASS_OFS_X"))
                                 ->rawValue()
                                 .toFloat()),
                "COMPASS_OFS_X not 0 on fresh-flash MockLink");
            QVERIFY2(
                qFuzzyIsNull(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("INS_ACCOFFS_X"))
                                 ->rawValue()
                                 .toFloat()),
                "INS_ACCOFFS_X not 0 on fresh-flash MockLink");
            QCOMPARE(
                mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC1_MIN"))->rawValue().toInt(),
                1100);
            QCOMPARE(
                mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC1_MAX"))->rawValue().toInt(),
                1900);
            QCOMPARE(
                mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC1_TRIM"))->rawValue().toInt(),
                1500);
            ...
            Fact* frameClassFact =
                mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("FRAME_CLASS"));
            ...
            // -------------------------------------------------------------------
            QVERIFY2(autopilot->prerequisiteSetup(sensorsComp).isEmpty(),
                     "Sensors still has a prerequisite after frame selection");
            QVERIFY2(autopilot->prerequisiteSetup(radioComp).isEmpty(),
                     "Radio still has a prerequisite after frame selection");
