...
void PlanViewUITest::_verifyWaypointToolAddsWaypointOnEmptyPlan(bool expectTakeoffButtonVisible)
{
    ...
    QVERIFY2(
        waitForCondition([&] { return _plannedHomePosition().isValid(); }, 2000, QStringLiteral("home position set")),
    ...
    QVERIFY2(waitForCondition([&] { return _missionItemCount() == 2; }, 2000, QStringLiteral("waypoint inserted")),
...
void PlanViewUITest::_testRoverWaypointOnEmptyPlan()
{
    ...
    SettingsManager::instance()->appSettings()->offlineEditingVehicleClass()->setRawValue(
...
void PlanViewUITest::_testTakeoffNotRequiredWaypointOnEmptyPlan()
{
    ...
    const auto restoreGuard =
        qScopeGuard([takeoffItemNotRequired, savedValue] { takeoffItemNotRequired->setRawValue(savedValue); });
...
void PlanViewUITest::_verifyFullState(const PlanUIState& state, const QString& context)
{
    ...
    if (state.templatesVisible) {
        ...
        QVERIFY2(waitForCondition([&] { return findVisibleItem(_rootItem, templatesCol, 0) == nullptr; }, 2000,
                                  QStringLiteral("templates section hidden")),
...
void PlanViewUITest::_testPlanViewStates()
{
    ...
    QVERIFY2(waitForCondition([&] { return QFile::exists(planFile); }, 5000, QStringLiteral("plan file written")),
...
void PlanViewUITest::_testSaveAsMenu()
{
    ...
    QVERIFY2(waitForCondition([&] { return _missionItemCount() == 2; }, 2000, QStringLiteral("takeoff item inserted")),
