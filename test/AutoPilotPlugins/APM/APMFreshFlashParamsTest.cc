...
void APMFreshFlashParamsTest::_testFreshFlashParams()
{
    ...
    // -------------------------------------------------------------------
    QVERIFY2(qFuzzyIsNull(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("COMPASS_OFS_X"))
                              ->rawValue()
                              .toFloat()),
             "COMPASS_OFS_X not 0 on fresh-flash MockLink");
    QVERIFY2(qFuzzyIsNull(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("INS_ACCOFFS_X"))
                              ->rawValue()
                              .toFloat()),
    ...
    for (const QString& mapParam : rcMapParams) {
        ...
        Fact* minFact =
            mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC%1_MIN").arg(channel));
        QCOMPARE(minFact->rawValue().toInt(), 1100);
        QCOMPARE(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC%1_MAX").arg(channel))
                     ->rawValue()
                     .toInt(),
                 1900);
        QCOMPARE(mgr->getParameter(ParameterManager::defaultComponentId, QStringLiteral("RC%1_TRIM").arg(channel))
                     ->rawValue()
                     .toInt(),
                 1500);
