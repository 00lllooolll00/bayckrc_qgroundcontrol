                          ...
                          const CancelToken& cancelToken = nullptr)
{
    ...
    const QString fileTypeDescription =
        suffix.isEmpty() ? LogFileParser::tr("no extension") : QStringLiteral(".%1").arg(suffix);

    LogParseResult result;
    result.errorMessage = LogFileParser::tr("Unsupported file type (%1) for file '%2'. Expected .bin, .log, or .ulg.")
...
LogFileParser::LogFileParser(QObject* parent) : QObject(parent)
{
...
bool LogFileParser::parseFile(const QString& filePath)
{
    ...
    _applyResult(result);
    qCDebug(LogFileParserLog) << "Parsed fields" << _availableFields.count() << "parameters" << _parameters.count()
...
void LogFileParser::startParsingAsync(const QString& filePath)
{
    ...
    auto progressCallback = [self, requestId](float v) {
        ...
        QMetaObject::invokeMethod(
            self.data(),
            [self, requestId, v]() {
                if (!self || requestId != self->_parseRequestId)
                    return;
    ...
    (void) connect(watcher, &QFutureWatcher<LogParseResult>::finished, this, [this, watcher, filePath, requestId]() {
...
void LogFileParser::_applyResult(const LogParseResult& result)
{
    ...
    if (!_parameters.isEmpty()) {
        if (result.sourceType == LogParseResult::SourceType::PX4ULog) {
            ...
            LogViewerParamMetaData::enrichForAPM(_parameters, result.detectedVehicleType, result.firmwareMajorVersion,
...
void LogFileParser::clear()
{
    ...
    if (!_modeNames.isEmpty()) {
        _modeNames.clear();
        _modeColorCache.clear();
        emit modeNamesChanged();
    }
    if (!_dropouts.isEmpty()) {
        _dropouts.clear();
        emit dropoutsChanged();
    }
...
QVariantList LogFileParser::fieldSamplesFiltered(const QString& fieldName, double minX, double maxX,
                                                 int pixelWidth) const
{
    QVariantList output;
    const auto it = _fieldSamples.constFind(fieldName);
    if (it == _fieldSamples.cend() || pixelWidth <= 0 || maxX <= minX) {
        return output;
    }
    ...
    const auto sliceBegin =
        std::lower_bound(points.cbegin(), points.cend(), minX, [](const QPointF& p, double t) { return p.x() < t; });
    const auto sliceEnd =
        std::upper_bound(sliceBegin, points.cend(), maxX, [](double t, const QPointF& p) { return t < p.x(); });
    ...
    if (sliceCount <= 4 * pixelWidth) {
        ...
        for (auto jt = sliceBegin; jt != sliceEnd; ++jt) {
            output.append(*jt);
        }
        return output;
    }
    ...
    auto columnOf = [&](double x) -> int { return static_cast<int>((x - minX) / range * pixelWidth); };
    ...
    for (qsizetype i = 0; i < sliceCount; ++i) {
        ...
        if (col != curCol) {
            ...
            if (p.y() < (sliceBegin + minIdx)->y())
                minIdx = i;
            if (p.y() > (sliceBegin + maxIdx)->y())
                maxIdx = i;
        }
        lastIdx = i;
    }
...
double LogFileParser::fieldValueAt(const QString& fieldName, double timestampSeconds) const
{
    ...
    return (std::fabs(prev->x() - timestampSeconds) <= std::fabs(lower->x() - timestampSeconds)) ? prev->y()
                                                                                                 : lower->y();
}
...
QVariantList LogFileParser::eventsNear(double timestampSeconds, double thresholdSeconds) const
{
    ...
    const auto lower = std::lower_bound(
        _events.cbegin(), _events.cend(), timestampSeconds - threshold,
        [](const QVariant& v, double t) { return v.toMap().value(QStringLiteral("time")).toDouble() < t; });
    for (auto it = lower; it != _events.cend(); ++it) {
        const QVariantMap ev = it->toMap();
        if (ev.value(QStringLiteral("time")).toDouble() > timestampSeconds + threshold) {
            break;
        }
        matches.append(ev);
    }
...
QVariantList LogFileParser::gpsPath() const
{
    ...
    static const CandidatePair candidates[] = {
        ...
        {"vehicle_global_position.latitude_deg", "vehicle_global_position.longitude_deg", "vehicle_global_position.alt",
         nullptr, 0},
        ...
        {"vehicle_gps_position.latitude_deg", "vehicle_gps_position.longitude_deg",
         "vehicle_gps_position.altitude_msl_m", nullptr, 0},
        {"vehicle_gps_position[0].latitude_deg", "vehicle_gps_position[0].longitude_deg",
         "vehicle_gps_position[0].altitude_msl_m", nullptr, 0},
    ...
    for (const auto& c : candidates) {
        ...
        qCDebug(LogFileParserLog) << "gpsPath: found candidate" << c.latField << "samples:" << latPts.size()
                                  << "first lat:" << latPts.first().y() << "first lon:" << lonPts.first().y();
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (lat < -90.0 || lat > 90.0 || lon < -180.0 || lon > 180.0 || (qFuzzyIsNull(lat) && qFuzzyIsNull(lon))) {
                continue;
            }
            QVariantMap coord;
    ...
    for (auto it = _fieldSamples.cbegin(); it != _fieldSamples.cend(); ++it) {
        ...
        if (fn.contains(QLatin1String("lat"), Qt::CaseInsensitive) ||
            fn.contains(QLatin1String("lon"), Qt::CaseInsensitive)) {
