...
QString _apmFileVehicleName(const QString& vehicleType)
{
    ...
    if (vehicleType == QStringLiteral("ArduRover")) {
        return QStringLiteral("Rover");
    }
    if (vehicleType == QStringLiteral("ArduSub")) {
...
QString _findAPMMetaDataFile(const QString& fileVehicleName, int major, int minor)
{
    ...
    for (int i = 0; i < 10; i++) {
        const QString file =
            QStringLiteral(":/FirmwarePlugin/APM/APMParameterFactMetaData.%1.4.%2.json").arg(fileVehicleName).arg(i);
        if (QFileInfo::exists(file)) {
            return file;
        }
    }
...
void _enrichRows(QVariantList& parameters, ParameterMetaData* metaData)
{
    for (int i = 0; i < parameters.size(); i++) {
        ...
        const FactMetaData::ValueType_t type = isFloat ? FactMetaData::valueTypeFloat : FactMetaData::valueTypeInt32;
...
void LogViewerParamMetaData::enrichForPX4(QVariantList& parameters)
{
    ...
    metaData->loadParameterFactMetaDataFile(QStringLiteral(":/FirmwarePlugin/PX4/PX4ParameterFactMetaData.json"));
...
void LogViewerParamMetaData::enrichForAPM(QVariantList& parameters, const QString& vehicleType, int major, int minor)
{
    if (parameters.isEmpty()) {
        return;
    }
    ...
    if (metaDataFile.isEmpty()) {
        qCDebug(LogViewerParamMetaDataLog) << "No APM metadata file found for" << vehicleType << major << "." << minor;
        return;
    }
