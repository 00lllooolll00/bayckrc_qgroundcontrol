...
SensorsComponent::SensorsComponent(Vehicle* vehicle, AutoPilotPlugin* autopilot, QObject* parent)
    : VehicleComponent(vehicle, autopilot, AutoPilotPlugin::KnownSensorsVehicleComponent, parent), _name(tr("Sensors"))
{
...
bool SensorsComponent::setupComplete(void) const
{
    for (const QString& triggerParam : std::as_const(_deviceIds)) {
        if (_vehicle->parameterManager()
                ->getParameter(ParameterManager::defaultComponentId, triggerParam)
                ->rawValue()
                .toFloat() == 0.0f) {
            return false;
        }
    ...
    if (_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, _magEnabledParam)) {
        magEnabled = _vehicle->parameterManager()
                         ->getParameter(ParameterManager::defaultComponentId, _magEnabledParam)
                         ->rawValue()
                         .toBool();
    }

    if (magEnabled && _vehicle->parameterManager()
                              ->getParameter(ParameterManager::defaultComponentId, _magCalParam)
                              ->rawValue()
                              .toFloat() == 0.0f) {
        return false;
    }
    ...
    if (_vehicle->fixedWing() || _vehicle->vtol() || _vehicle->airship()) {
        if (_vehicle->firmwareMajorVersion() > 1 ||
            (_vehicle->firmwareMajorVersion() == 1 && _vehicle->firmwareMinorVersion() > 14)) {
            if (_vehicle->parameterManager()
                    ->getParameter(ParameterManager::defaultComponentId, "SYS_HAS_NUM_ASPD")
                    ->rawValue()
                    .toBool() &&
                _vehicle->parameterManager()
                        ->getParameter(ParameterManager::defaultComponentId, "SENS_DPRES_OFF")
                        ->rawValue()
                        .toFloat() == 0.0f) {
                return false;
            }
            ...
            if (!_vehicle->parameterManager()
                     ->getParameter(ParameterManager::defaultComponentId, "FW_ARSP_MODE")
                     ->rawValue()
                     .toBool() &&
                _vehicle->parameterManager()
                        ->getParameter(ParameterManager::defaultComponentId, "CBRK_AIRSPD_CHK")
                        ->rawValue()
                        .toInt() != 162128 &&
                _vehicle->parameterManager()
                        ->getParameter(ParameterManager::defaultComponentId, "SENS_DPRES_OFF")
...
QStringList SensorsComponent::sectionIds() const
{
    ...
    if (_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, _magEnabledParam)) {
        allMagsDisabled = !_vehicle->parameterManager()
                               ->getParameter(ParameterManager::defaultComponentId, _magEnabledParam)
                               ->rawValue()
                               .toBool();
    }
...
{
    return _airspeedCalSupported() && _vehicle->parameterManager()
                                              ->getParameter(ParameterManager::defaultComponentId, "SENS_DPRES_OFF")
                                              ->rawValue()
                                              .toFloat() == 0.0f;
}
