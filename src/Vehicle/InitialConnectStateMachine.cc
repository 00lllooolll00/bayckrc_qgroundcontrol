...
void InitialConnectStateMachine::_createStates()
{
    ...
    _stateAutopilotVersion = new RetryableRequestMessageState(
        QStringLiteral("RequestAutopilotVersion"), this, MAVLINK_MSG_ID_AUTOPILOT_VERSION,
        [this](Vehicle*, const mavlink_message_t& message) { _handleAutopilotVersionSuccess(message); },
        _autopilotVersionMaxRetries, MAV_COMP_ID_AUTOPILOT1);
    _stateAutopilotVersion->setSkipPredicate([this]() { return _shouldSkipAutopilotVersionRequest(); });
    _stateAutopilotVersion->setFailureHandler([this](auto, auto) { _handleAutopilotVersionFailure(); });
    ...
    _stateStandardModes = new AsyncFunctionState(QStringLiteral("RequestStandardModes"), this,
    ...
    _stateCompInfo = new AsyncFunctionState(QStringLiteral("RequestCompInfo"), this,
    ...
    _stateMission = new SkippableAsyncState(
        QStringLiteral("RequestMission"), this, [this]() { return _shouldSkipForPlanLoad(); },
        [this](SkippableAsyncState* state) { _requestMission(state); },
        [this]() { qCDebug(InitialConnectStateMachineLog) << "Skipping mission load" << _lastSkipReason; });

    // State 5: Request geofence (skippable)
    // No timeout: PlanManager handles all timeouts/retries internally and always signals completion.
    _stateGeoFence = new SkippableAsyncState(
        QStringLiteral("RequestGeoFence"), this,
        [this]() {
        ...
        [this]() { qCDebug(InitialConnectStateMachineLog) << "Skipping geofence load" << _lastSkipReason; });

    // State 6: Request rally points (skippable)
    // No timeout: PlanManager handles all timeouts/retries internally and always signals completion.
    _stateRallyPoints = new SkippableAsyncState(
        QStringLiteral("RequestRallyPoints"), this,
        [this]() {
            if (_shouldSkipForPlanLoad()) {
                return true;
            }
    ...
    _stateComplete = new RetryState(
        QStringLiteral("SignalComplete"), this,
        [this]() {
            _signalComplete();
            return true;
        },
        0, 0, RetryState::EmitError);
...
void InitialConnectStateMachine::_wireProgressTracking()
{
    ...
    setProgressWeights({{_stateAutopilotVersion, 1},
...
void InitialConnectStateMachine::_handleAutopilotVersionSuccess(const mavlink_message_t& message)
{
    ...
    if (vehicle()->px4Firmware()) {
        ...
        // PX4 Firmware stores the first 16 characters of the git hash as binary, with the individual bytes in reverse
        // order
        vehicle()->_gitHash = "";
        for (int i = 7; i >= 0; i--) {
...
void InitialConnectStateMachine::_requestCompInfo(AsyncFunctionState* state)
{
    ...
    connect(vehicle()->_componentInformationManager, &ComponentInformationManager::progressUpdate, this,
            &InitialConnectStateMachine::_onSubProgressUpdate, Qt::UniqueConnection);

    // Ensure progress tracking is always cleaned up, including timeout/skip paths.
    state->setOnExit([this]() {
        disconnect(vehicle()->_componentInformationManager, &ComponentInformationManager::progressUpdate, this,
                   &InitialConnectStateMachine::_onSubProgressUpdate);
    });

    state->connectToCompletion(vehicle()->_componentInformationManager,
                               &ComponentInformationManager::requestAllComplete);
...
void InitialConnectStateMachine::_requestParameters(SkippableAsyncState* state)
{
    ...
    connect(vehicle()->_parameterManager, &ParameterManager::loadProgressChanged, this,
            &InitialConnectStateMachine::_onSubProgressUpdate, Qt::UniqueConnection);

    // If the vehicle never answers PARAM_REQUEST_LIST, advance without parameters
    const QMetaObject::Connection requestFailedConn =
        connect(vehicle()->_parameterManager, &ParameterManager::initialParametersRequestFailed, state, [state]() {
    ...
    state->connectToCompletion(vehicle()->_parameterManager, &ParameterManager::parametersReadyChanged,
                               [this](bool parametersReady) { _onParametersReady(parametersReady); });
    ...
    state->setOnExit([this, cacheFailedConn, requestFailedConn]() {
        disconnect(vehicle()->_parameterManager, &ParameterManager::loadProgressChanged, this,
                   &InitialConnectStateMachine::_onSubProgressUpdate);
...
void InitialConnectStateMachine::_requestMission(SkippableAsyncState* state)
{
    ...
    connect(vehicle()->_missionManager, &MissionManager::progressPctChanged, this,
            &InitialConnectStateMachine::_onSubProgressUpdate, Qt::UniqueConnection);
    ...
    state->setOnExit([this]() {
        disconnect(vehicle()->_missionManager, &MissionManager::progressPctChanged, this,
                   &InitialConnectStateMachine::_onSubProgressUpdate);
    });
...
void InitialConnectStateMachine::_requestGeoFence(SkippableAsyncState* state)
{
    ...
    connect(vehicle()->_geoFenceManager, &GeoFenceManager::progressPctChanged, this,
            &InitialConnectStateMachine::_onSubProgressUpdate, Qt::UniqueConnection);
    ...
    // Disconnect progress tracking on exit
    state->setOnExit([this]() {
        disconnect(vehicle()->_geoFenceManager, &GeoFenceManager::progressPctChanged, this,
                   &InitialConnectStateMachine::_onSubProgressUpdate);
    });
...
void InitialConnectStateMachine::_requestRallyPoints(SkippableAsyncState* state)
{
    ...
    connect(vehicle()->_rallyPointManager, &RallyPointManager::progressPctChanged, this,
            &InitialConnectStateMachine::_onSubProgressUpdate, Qt::UniqueConnection);

    state->connectToCompletion(vehicle()->_rallyPointManager, &RallyPointManager::loadComplete, [this]() {
    ...
    state->setOnExit([this]() {
        disconnect(vehicle()->_rallyPointManager, &RallyPointManager::progressPctChanged, this,
                   &InitialConnectStateMachine::_onSubProgressUpdate);
    });
...
void InitialConnectStateMachine::_signalComplete()
{
    ...
    connect(
        this, &QStateMachine::finished, vehicle(), [this]() { emit vehicle() -> initialConnectComplete(); },
