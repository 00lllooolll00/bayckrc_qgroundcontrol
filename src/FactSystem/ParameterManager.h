...
public:
    ...
    static constexpr int kTestHashCheckTimeoutMs =
        200;  ///< Shortened _HASH_CHECK timeout in unit tests (MockLink responds instantly)
...
private:
    ...
    ///     @param waitingParamTimeout: true: being called due to timeout, false: being called to re-fill the batch
    ///     queue
    ...
    static constexpr int _maxInitialLoadRetrySingleParam =
    ...
    bool _indexBatchQueueActive = false;  ///< true: we are actively batching re-requests for missing index base params,
                                          ///< false: index based re-request has not yet started
    ...
    QMap<int, QMap<int, int>> _waitingReadParamIndexMap;  ///< Key: Component id, Value: Map { Key: parameter index
