...
QGC_LOGGING_CATEGORY(ParameterManagerDebugCacheFailureLog,
                     "FactSystem.ParameterManager:debugCacheFailure")  // Turn on to debug parameter cache crc misses
                                         ...
                                         const QVariant& parameterValue)
{
    ...
    if ((parameterIndex == 65535) && (parameterName != QStringLiteral("_HASH_CHECK")) &&
        _paramRequestListTimer.isActive()) {
    ...
    if (!_initialLoadComplete && !_logReplay && _debugCacheCRC.contains(componentId) && _debugCacheCRC[componentId]) {
        if (_debugCacheMap[componentId].contains(parameterName)) {
            ...
            const size_t dataSize =
                FactMetaData::typeToSize(static_cast<FactMetaData::ValueType_t>(cacheParamTypeVal.first));
            ...
            if (memcmp(cacheData, vehicleData, dataSize) != 0) {
                qCDebug(ParameterManagerVerbose1Log) << "Cache/Vehicle values differ for name:cache:actual"
                                                     << parameterName << parameterValue << cacheParamTypeVal.second;
            }
...
void ParameterManager::_mavlinkParamSet(int componentId, const QString& paramName, FactMetaData::ValueType_t valueType,
                                        const QVariant& rawValue)
{
    auto paramSetEncoder = [this, componentId, paramName, valueType, rawValue](uint8_t /*systemId*/, uint8_t channel,
        ...
        (void) mavlink_msg_param_set_pack_chan(MAVLinkProtocol::instance()->getSystemId(),
                                               ...
                                               static_cast<uint8_t>(_vehicle->id()), static_cast<uint8_t>(componentId),
                                               paramId, union_value.param_float, static_cast<uint8_t>(paramType));
    };

    auto checkForCorrectParamValue = [this, componentId, paramName,
                                      rawValue](const mavlink_message_t& message) -> bool {
        ...
        // Check that the value matches what we expect within tolerance, if it doesn't match then this message is not
        // for us
        ...
        if (rawValue.typeId() != receivedValue.typeId()) {
            qCWarning(ParameterManagerLog)
                << "QVariant type mismatch on PARAM_VALUE ack for" << paramName << ": expected type"
                << rawValue.typeId() << "got type" << receivedValue.typeId();
            return false;
        }
    ...
    auto incPendingWriteCountState = new FunctionState(QStringLiteral("ParameterManager increment pending write count"),
                                                       stateMachine, [this]() { _incrementPendingWriteCount(); });
    auto decPendingWriteCountState = new FunctionState(QStringLiteral("ParameterManager decrement pending write count"),
                                                       stateMachine, [this]() { _decrementPendingWriteCount(); });
    auto retryDecPendingWriteCountState =
        new FunctionState(QStringLiteral("ParameterManager retry decrement pending write count"), stateMachine,
                          [this]() { _decrementPendingWriteCount(); });
    auto errorDecPendingWriteCountState =
        new FunctionState(QStringLiteral("ParameterManager error decrement pending write count"), stateMachine,
                          [this]() { _decrementPendingWriteCount(); });
    auto waitAckState = new WaitForParamResponseState(stateMachine, _waitForParamValueAckMs, checkForCorrectParamValue,
                                                      checkForParamError);
    auto paramRefreshState = new FunctionState(
        QStringLiteral("ParameterManager param refresh"), stateMachine, [this, waitAckState, componentId, paramName]() {
            ...
            if (waitAckState->lastParamError() == MAV_PARAM_ERROR_DOES_NOT_EXIST) {
                qCDebug(ParameterManagerLog)
                    << "Skipping post-write-failure refresh, param does not exist on vehicle:" << paramName
                    << _vehicleAndComponentString(componentId);
                return;
            }
            refreshParameter(componentId, paramName);
        });
    auto userNotifyState = new FunctionState(
        QStringLiteral("ParameterManager user notify"), stateMachine, [waitAckState, paramName, this, componentId]() {
            const QString errorDetail = waitAckState->lastParamErrorString();
            const QString msg = errorDetail.isEmpty()
                                    ? QStringLiteral("Parameter write failed: param: %1 %2")
                                          .arg(paramName, _vehicleAndComponentString(componentId))
                                    : QStringLiteral("Parameter write failed: param: %1 %2 - %3")
                                          .arg(paramName, _vehicleAndComponentString(componentId), errorDetail);
            QGC::showAppMessage(msg);
        });
    auto logSuccessState = new FunctionState(
        QStringLiteral("ParameterManager log success"), stateMachine, [this, componentId, paramName]() {
            qCDebug(ParameterManagerLog) << "Parameter write succeeded: param:" << paramName
                                         << _vehicleAndComponentString(componentId);
            emit _paramSetSuccess(componentId, paramName);
        });
    auto logFailureState = new FunctionState(
        QStringLiteral("ParameterManager log failure"), stateMachine, [this, componentId, paramName]() {
            qCDebug(ParameterManagerLog) << "Parameter write failed: param:" << paramName
                                         << _vehicleAndComponentString(componentId);
    ...
    qCDebug(ParameterManagerLog) << "Starting state machine for PARAM_SET on: " << paramName
                                 << _vehicleAndComponentString(componentId);
    stateMachine->start();
}

bool ParameterManager::_fillMavlinkParamUnion(FactMetaData::ValueType_t valueType, const QVariant& rawValue,
...
void ParameterManager::_ftpDownloadComplete(const QString& fileName, const QString& errorMsg)
{
    ...
    (void) disconnect(_vehicle->ftpManager(), &FTPManager::downloadComplete, this,
                      &ParameterManager::_ftpDownloadComplete);
    (void) disconnect(_vehicle->ftpManager(), &FTPManager::commandProgress, this,
                      &ParameterManager::_ftpDownloadProgress);
...
bool ParameterManager::_fillIndexBatchQueue(bool waitingParamTimeout)
{
    ...
    for (const int componentId : _waitingReadParamIndexMap.keys()) {
        if (_waitingReadParamIndexMap[componentId].count()) {
            ...
            qCDebug(ParameterManagerVerbose1Log)
                << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap (index, retry count)"
...
void ParameterManager::_waitingParamTimeout()
{
    ...
    qCDebug(ParameterManagerLog) << _logVehiclePrefix(-1) << "_waitingParamTimeout after"
                                 << _waitingParamTimeoutTimer.interval() << "ms";
...
void ParameterManager::_requestHashCheck(uint8_t componentId)
{
    ...
    (void) mavlink_msg_param_request_read_pack_chan(
        ...
        &msg, static_cast<uint8_t>(_vehicle->id()), componentId, paramId, -1);

    (void) _vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
}

void ParameterManager::_mavlinkParamRequestRead(int componentId, const QString& paramName, int paramIndex,
                                                bool notifyFailure)
{
    auto paramRequestReadEncoder = [this, componentId, paramName, paramIndex](uint8_t /*systemId*/, uint8_t channel,
                                                                              mavlink_message_t* message) -> void {
        char paramId[MAVLINK_MSG_PARAM_REQUEST_READ_FIELD_PARAM_ID_LEN + 1] = {};
        ...
        (void) mavlink_msg_param_request_read_pack_chan(MAVLinkProtocol::instance()->getSystemId(),  // QGC system id
                                                        ...
                                                        channel, message, static_cast<uint8_t>(_vehicle->id()),
    ...
    auto sendParamRequestReadState =
        new SendMavlinkMessageState(stateMachine, paramRequestReadEncoder, kParamRequestReadRetryCount);
    auto waitAckState = new WaitForParamResponseState(stateMachine, _waitForParamValueAckMs, checkForCorrectParamValue,
                                                      checkForParamError);
    auto userNotifyState =
        new FunctionState(QStringLiteral("User notify"), stateMachine, [waitAckState, paramName, this, componentId]() {
            const QString errorDetail = waitAckState->lastParamErrorString();
            const QString msg = errorDetail.isEmpty()
                                    ? QStringLiteral("Parameter read failed: param: %1 %2")
                                          .arg(paramName, _vehicleAndComponentString(componentId))
                                    : QStringLiteral("Parameter read failed: param: %1 %2 - %3")
                                          .arg(paramName, _vehicleAndComponentString(componentId), errorDetail);
            QGC::showAppMessage(msg);
        });
    auto logSuccessState =
        new FunctionState(QStringLiteral("Log success"), stateMachine, [this, componentId, paramName, paramIndex]() {
            qCDebug(ParameterManagerLog) << "PARAM_REQUEST_READ succeeded: name:" << paramName << "index" << paramIndex
                                         << _vehicleAndComponentString(componentId);
            emit _paramRequestReadSuccess(componentId, paramName, paramIndex);
        });
    auto logFailureState =
        new FunctionState(QStringLiteral("Log failure"), stateMachine, [this, componentId, paramName, paramIndex]() {
            qCDebug(ParameterManagerLog) << "PARAM_REQUEST_READ failed: param:" << paramName << "index" << paramIndex
                                         << _vehicleAndComponentString(componentId);
    ...
    qCDebug(ParameterManagerLog) << "Starting state machine for PARAM_REQUEST_READ on: " << paramName
                                 << _vehicleAndComponentString(componentId);
    stateMachine->start();
}
...
void ParameterManager::_writeLocalParamCache(int vehicleId, int componentId)
{
    ...
    if (cacheFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        ...
        ds << cacheMap;
        qCDebug(ParameterManagerLog) << "Parameter cache written" << cacheFile.fileName()
                                     << "paramCount:" << cacheMap.count();
    } else {
...
void ParameterManager::_tryCacheHashLoad(int vehicleId, int componentId, const QVariant& hashValue)
{
    ...
    for (const QString& name : cacheMap.keys()) {
        ...
        if (_vehicle->compInfoManager()
                ...
                ->factMetaDataForName(name, factType)
                ->volatileValue()) {
            ...
            crc32_value =
                QGC::crc32(static_cast<const uint8_t*>(vdat), FactMetaData::typeToSize(cacheFactType), crc32_value);
        }
    }
    ...
    if (crc32_value == hashValue.toUInt()) {
        ...
        connect(ani, &QVariantAnimation::finished, this,
                [this] { QTimer::singleShot(500, this, [this] { _setLoadProgress(0); }); });

        ani->start(QAbstractAnimation::DeleteWhenStopped);
    } else {
        qCDebug(ParameterManagerLog) << "Parameters cache match failed"
                                     << qPrintable(QFileInfo(cacheFile).absoluteFilePath());
...
void ParameterManager::writeParametersToStream(QTextStream& stream) const
{
    ...
    stream << "# Stack: "
           << QGCMAVLink::firmwareClassToCanonicalString(QGCMAVLink::firmwareClass(_vehicle->firmwareType())) << "\n";
    stream << "# Vehicle: "
           << QGCMAVLink::vehicleClassToCanonicalString(QGCMAVLink::vehicleClass(_vehicle->vehicleType())) << "\n";
        ...
        for (const QString& paramName : _mapCompId2FactMap[componentId].keys()) {
            ...
            if (fact) {
                stream << _vehicle->id() << "\t" << componentId << "\t" << paramName << "\t"
                       ...
                       << QStringLiteral("%1").arg(factTypeToMavType(fact->type())) << "\n";
            } else {
...
void ParameterManager::_paramRequestListTimeout()
{
    ...
    qCDebug(ParameterManagerLog) << _logVehiclePrefix(-1)
                                 << "Initial parameter request list retries exhausted, giving up";
...
void ParameterManager::_loadOfflineEditingParams()
{
    ...
    while (!paramStream.atEnd()) {
        ...
        FactMetaData* const factMetaData = _vehicle->compInfoManager()
                                               ->compInfoParam(offlineDefaultComponentId)
...
bool ParameterManager::_parseParamFile(const QString& filename)
{
    ...
    while (in.status() == QDataStream::Ok) {
        ...
        while (byte == 0x0) {  // Eat padding bytes
            ...
            if (in.status() != QDataStream::Ok) {
                if (no_of_parameters_found == num_params) {
                    ...
                    qCDebug(ParameterManagerLog)
                        << "_parseParamFile: Error: unexpected EOF" << "number of parameters expected:" << num_params
        ...
        if ((name_len + common_len) > 16) {
            qCCritical(ParameterManagerLog) << "_parseParamFile: Error: common_len + name_len > 16" << "name_len"
                                            << name_len << "common_len" << common_len;
            goto Error;
        }
