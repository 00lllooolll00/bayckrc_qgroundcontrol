      ...
      _appSettings(SettingsManager::instance()->appSettings())
{
    ...
    connect(_planViewSettings->takeoffItemNotRequired(), &Fact::rawValueChanged, this,
            &MissionController::_recalcPlanViewState);
    ...
    if (_flyView || removeAllRequested || _itemsRequested || isEmpty()) {
        ...
        for (; i < newMissionItems.count(); i++) {
            ...
            if (TakeoffMissionItem::isTakeoffCommand(static_cast<MAV_CMD>(simpleItem->command()))) {
                ...
                _takeoffMissionItem = new TakeoffMissionItem(*missionItem, _masterController, _flyView, _settingsItem,
                                                             false /* forLoad */);
        ...
        // We set Altitude frame to mixed, otherwise if we need a non relative altitude frame we won't be able to change
        // it
        setGlobalAltitudeFrame(weHaveItemsFromVehicle ? QGroundControlQmlGlobal::AltitudeFrameMixed
        ...
        if (!MissionCommandTree::instance()->isLandCommand(command)) {
            ...
            if (_findPreviousAltitude(visualItemIndex, &prevAltitude, &prevAltFrame)) {
                ...
                if (globalAltitudeFrame() == QGroundControlQmlGlobal::AltitudeFrameMixed) {
                    // We are in mixed altitude frames, so copy from previous. Otherwise altitude frame will be set from
                    // global setting.
...
bool MissionController::multipleLandPatternsAllowed(void) const
{
    ...
    return (_controllerVehicle->fixedWing() || _controllerVehicle->vtol()) &&
           _planViewSettings->allowMultipleLandingPatterns()->rawValue().toBool() &&
                                                     ...
                                                     bool makeCurrentItem)
{
    if (_controllerVehicle->fixedWing()) {
        FixedWingLandingComplexItem* fwLanding = qobject_cast<FixedWingLandingComplexItem*>(insertComplexMissionItem(
            FixedWingLandingComplexItem::canonicalName, coordinate, visualItemIndex, makeCurrentItem));
        return fwLanding;
    } else if (_controllerVehicle->vtol()) {
        VTOLLandingComplexItem* vtolLanding = qobject_cast<VTOLLandingComplexItem*>(insertComplexMissionItem(
            VTOLLandingComplexItem::canonicalName, coordinate, visualItemIndex, makeCurrentItem));
        return vtolLanding;
    } else {
        return _insertSimpleMissionItemWorker(coordinate, MAV_CMD_NAV_RETURN_TO_LAUNCH, visualItemIndex,
                                              makeCurrentItem);
    }
}
                                                               ...
                                                               int visualItemIndex, bool makeCurrentItem)
{
    ComplexMissionItem* newItem =
        QGCCorePlugin::instance()->createComplexMissionItem(itemName, _masterController, _flyView);
    if (!newItem) {
        return nullptr;
    }
...
{
    ComplexMissionItem* newItem =
        QGCCorePlugin::instance()->createComplexMissionItem(itemName, _masterController, _flyView, file);
    if (!newItem) {
        return nullptr;
    }
                                               ...
                                               QString& errorString)
{
    ...
    if (json.contains(_jsonGlobalPlanAltitudeModeKey)) {
        setGlobalAltitudeFrame(
            json[_jsonGlobalPlanAltitudeModeKey].toVariant().value<QGroundControlQmlGlobal::AltitudeFrame>());
    }

    QGeoCoordinate homeCoordinate;
    if (!GeoJsonHelper::loadGeoCoordinate(json[_jsonPlannedHomePositionKey], true /* altitudeRequired */,
    ...
    for (int i = 0; i < rgMissionItems.count(); i++) {
        ...
        if (itemType == VisualMissionItem::jsonTypeSimpleItemValue) {
            ...
            qCDebug(MissionControllerLog)
                << "Loading complex item type:" << complexItemType << "nextSequenceNumber:" << nextSequenceNumber;
            ComplexMissionItem* complexItem =
                QGCCorePlugin::instance()->createComplexMissionItem(complexItemType, _masterController, _flyView);
            if (!complexItem) {
                ...
                if (firstItem && plannedHomePositionInFile) {
                    ...
                    if (TakeoffMissionItem::isTakeoffCommand(static_cast<MAV_CMD>(item->command()))) {
                        ...
                        TakeoffMissionItem* takeoffItem = new TakeoffMissionItem(
                            item->missionItem(), _masterController, _flyView, settingsItem, false /* forLoad */);
                        item->deleteLater();
                        item = takeoffItem;
                    }
...
FlightPathSegment* MissionController::_createFlightPathSegmentWorker(VisualItemPair& pair, bool mavlinkTerrainFrame)
{
    ...
    connect(pair.second, &VisualMissionItem::entryCoordinateChanged, this,
            &MissionController::_recalcMissionFlightStatusSignal, Qt::QueuedConnection);
    connect(pair.second, &VisualMissionItem::exitCoordinateChanged, this,
            &MissionController::_recalcMissionFlightStatusSignal, Qt::QueuedConnection);
            ...
            if (complexItem && complexItem->isIncomplete()) {
                ...
                if (lastFlyThroughVI != _settingsItem || (homePositionValid && linkStartToHome)) {
                    ...
                    bool mavlinkTerrainFrame =
                        lastSimpleItem ? lastSimpleItem->missionItem().frame() == MAV_FRAME_GLOBAL_TERRAIN_ALT : false;
                    FlightPathSegment* segment =
    ...
    if (lastSegmentVisualItemPair.first) {
        ...
        if (_flightPathSegmentHashTable.contains(lastSegmentVisualItemPair)) {
            ...
            coordVector = new FlightPathSegment(
                ...
                lastSegmentVisualItemPair.first->isSimpleItem() ? lastSegmentVisualItemPair.first->entryCoordinate()
                ...
                lastSegmentVisualItemPair.second->entryCoordinate(), lastSegmentVisualItemPair.second->amslEntryAlt(),
...
void MissionController::_recalcMissionFlightStatus()
{
    ...
    _flightStatusCalc.recalc(_visualItems, _settingsItem, _controllerVehicle, _managerVehicle, _appSettings,
                             _planViewSettings, _missionContainsVTOLTakeoff);
    _missionFlightStatus = _flightStatusCalc.status();
...
void MissionController::_setupTreeModel(void)
{
    ...
    _visualItemsTree.appendItem(&_planFileInfoMarker, _visualItemsTree.indexForObject(&_planFileGroupNode),
                                QStringLiteral("planFileInfo"));
    ...
    _visualItemsTree.appendItem(&_defaultsInfoMarker, _visualItemsTree.indexForObject(&_defaultsGroupNode),
                                QStringLiteral("defaultsInfo"));
    ...
    _visualItemsTree.appendItem(&_fenceEditorMarker, _visualItemsTree.indexForObject(&_fenceGroupNode),
                                QStringLiteral("fenceEditor"));
    ...
    _visualItemsTree.appendItem(&_rallyHeaderMarker, _visualItemsTree.indexForObject(&_rallyGroupNode),
    ...
    _visualItemsTree.appendItem(&_transformEditorMarker, _visualItemsTree.indexForObject(&_transformGroupNode),
                                QStringLiteral("transformEditor"));
...
void MissionController::_syncTreeRallyPointsInserted(const QModelIndex& parent, int first, int last)
{
    ...
    if (first == 0 && _visualItemsTree.rowCount(_rallyGroupIndex) == 1) {
        ...
        if (_visualItemsTree.data(child, QmlObjectTreeModel::NodeTypeRole).toString() ==
            QStringLiteral("rallyHeader")) {
...
void MissionController::_initAllVisualItems(void)
{
    ...
    connect(_visualItems, &QAbstractItemModel::rowsAboutToBeRemoved, this,
            &MissionController::_syncTreeMissionItemsAboutToBeRemoved);
    ...
    if (rallyController) {
        ...
        connect(pts, &QAbstractItemModel::rowsAboutToBeRemoved, this,
                &MissionController::_syncTreeRallyPointsAboutToBeRemoved);
...
void MissionController::_deinitAllVisualItems(void)
{
    ...
    disconnect(_settingsItem, &MissionSettingsItem::coordinateChanged, this,
               &MissionController::plannedHomePositionChanged);
    disconnect(_settingsItem, &MissionSettingsItem::coordinateChanged, this,
               &MissionController::homePositionSetChanged);
    ...
    disconnect(_visualItems, &QAbstractItemModel::rowsInserted, this,
               &MissionController::_syncTreeMissionItemsInserted);
    disconnect(_visualItems, &QAbstractItemModel::rowsAboutToBeRemoved, this,
               &MissionController::_syncTreeMissionItemsAboutToBeRemoved);
    ...
    if (rallyController) {
        ...
        disconnect(pts, &QAbstractItemModel::rowsAboutToBeRemoved, this,
                   &MissionController::_syncTreeRallyPointsAboutToBeRemoved);
...
bool MissionController::_findPreviousAltitude(int newIndex, double* prevAltitude,
                                              QGroundControlQmlGlobal::AltitudeFrame* prevAltitudeMode)
{
    bool found = false;
...
void MissionController::setCurrentPlanViewSeqNum(int sequenceNumber, bool force)
{
    if (_visualItems && (force || sequenceNumber != _currentPlanViewSeqNum)) {
        ...
        if (_masterController->controllerVehicle()->supports()->takeoffMissionCommand() &&
...
void MissionController::repositionMission(const QGeoCoordinate& newHome, bool repositionTakeoffItems,
                                          bool repositionLandingItems)
{
    ...
    for (int i = 0; i < _visualItems->count(); ++i) {
        ...
        if ((!repositionTakeoffItems && item->isTakeoffItem()) || (!repositionLandingItems && item->isLandCommand())) {
            continue;
        }
...
void MissionController::offsetMission(double eastMeters, double northMeters, double upMeters, bool offsetTakeoffItems,
                                      bool offsetLandingItems)
{
    if (!qFuzzyIsNull(eastMeters) || !qFuzzyIsNull(northMeters)) {
        ...
        for (int i = 0; i < _visualItems->count(); ++i) {
            ...
            if ((!offsetTakeoffItems && item->isTakeoffItem()) || (!offsetLandingItems && item->isLandCommand())) {
                continue;
            }
...
void MissionController::rotateMission(double degreesCW, bool rotateTakeoffItems, bool rotateLandingItems)
{
    ...
    for (int i = 0; i < _visualItems->count(); ++i) {
        ...
        if ((!rotateTakeoffItems && item->isTakeoffItem()) || (!rotateLandingItems && item->isLandCommand())) {
            continue;
        }
